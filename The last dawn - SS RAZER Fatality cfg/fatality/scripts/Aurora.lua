...
print("| Aurora Debug Loaded                 |")
print("| Version: 1.2                        |")
print("| Dev: Society#7777                   |")
print("|_____________________________________|")
...
local function WM()
    ...
    local watermarkText = ' Aurora [Debug] / Version 1.2 / ' .. realtime .. ' time / Delay: ' .. latency .. 'ms';
...
if not engine.is_in_game() then return end
    ...
    local binds =
    {
        ...
        Find("rage>aimbot>aimbot>force safepoint"):get_bool(),
...
if indicatorsmain:get_int() == 1 then
    ...
    local SP = Find("rage>aimbot>aimbot>force safepoint"):get_bool()
    ...
    local health = p:get_prop("m_iHealth")
        ...
        if ragebotlogs:get_bool() then
            if shot.server_damage > 0 then
                print( "[Aurora] Hurt " , n.name  , "'s ", hitgroup_names[hitgroup + 1]," for " , shot.server_damage, " damage [hc=", math.floor(shot.hitchance), ", bt=", math.floor(shot.backtrack),"]")
            else
                print( "[Aurora] Missed " , n.name  , "'s ", hitgroup_names[shot.client_hitgroup + 1]," due to ", shot.result)
            end
        end
